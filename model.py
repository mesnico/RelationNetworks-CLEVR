...
class RN(BasicModel):
    def __init__(self, args):
        ...
        self.wembedding = nn.Embedding(args.qdict_size+1, 32, padding_idx=0)  #word embeddings have size 32. Indexes 0 output 0 vectors (variable len questions)
        self.lstm = nn.LSTM(32, self.qembed_size, self.lstm_layers)  # Input dim is 32, output dim is the question embedding. 128 recurrent layers
        ...
        self.g_fc1 = nn.Linear((24+2)*2+self.qembed_size, 512)
    ...
    def init_lstm_hidden(self):
        hidden_state = torch.randn(self.lstm_layers, self.args.batch_size, self.qembed_size)
        cell_state = torch.randn(self.lstm_layers, self.args.batch_size, self.qembed_size)
        if self.args.cuda:
    ...
    def forward(self, img, qst_idxs):
        ...
        lstm_out, self.hidden = self.lstm(wembed, self.hidden)
        qst = lstm_out[-1]  # last layer of the lstm. qst = (64 x 32)
        ...
        qst = torch.unsqueeze(qst, 1)   #(64 x 1 x 32)
        qst = qst.repeat(1,self.conv_out_size**2,1) #(64 x 64 x 32)
        qst = torch.unsqueeze(qst, 2)   #(64 x 64 x 1 x 32)
        ...
        x_j = x_j.repeat(1,1,self.conv_out_size**2,1) # (64 x 64 x 64 x 26+32)
        
        # concatenate all together
        x_full = torch.cat([x_i,x_j],3) # (64 x 64 x 64 x 2*26+32)
        
        # reshape for passing through network
        x_ = x_full.view(mb*d*d*d*d,2*26+self.qembed_size)
        x_ = self.g_fc1(x_)
        x_ = F.relu(x_)
        x_ = self.g_fc2(x_)
        x_ = F.relu(x_)
